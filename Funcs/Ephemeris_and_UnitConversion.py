...
spice.furnsh(
    "/home/ouyangjiahong/codes/CRTBP_code/Spice/Kernel/metakernel/EarthMoon.mk"
)
...
def Ephemeris(observer, target, frame, et):
    ...
    if len(et) == 1:
        ...
        results = []
        for et_i in et:
            State_i, LightTime = spice.spkezr(
                target, float(et_i), frame, "None", observer
            )
...
def Normalization_to_Normal(
    primary1, primary2, et0, t_Normalization, r_Normalization, v_Normalization
):
...
def Normalization_to_True(
    ...
    # 获取真实单位下的位置速度矢量
    r_True = r_Normalization.reshape((np.size(et)), 1, 3) * l_star.reshape(
        (np.size(et)), 1, 1
    )
    v_True = v_Normalization.reshape((np.size(et)), 1, 3) * v_star.reshape(
        (np.size(et)), 1, 1
    )
...
def True_to_Normalization(primary1, primary2, t_Normalization0, et, r_True, v_True):
    ...
    for i in range(0, np.size(et)):
        [r_Primary2_i, v_Primary2_i] = Ephemeris(
            primary1, primary2, "J2000", np.array([et_last])
        )
        l_star_i = np.linalg.norm(r_Primary2_i, ord=None, axis=1)
        t_star_i = (l_star_i**3 / (GM_Primary1 + GM_Primary2)) ** 0.5
    ...
    # 获取真实单位下的位置速度矢量
    r_Normalization = r_True.reshape((np.size(et)), 1, 3) / l_star.reshape(
        (np.size(et)), 1, 1
    )
    v_Normalization = v_True.reshape((np.size(et)), 1, 3) / v_star.reshape(
        (np.size(et)), 1, 1
    )
