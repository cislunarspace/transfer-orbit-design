...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m plot.inspection.plot_interactive_orbit_inspector --help
"""
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="交互式轨道检查器", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
...
# - 直跑 ``python -m plot.inspection.plot_interactive_orbit_inspector``
...
def main():
    ...
    parser_obj = argparse.ArgumentParser(
        prog="plot_interactive_orbit_inspector", description="交互式轨道检查器"
    )
    try:
    ...
    while True:
        ...
        # 根据Jacobi值计算颜色
        norm_jacobi = (
            (jacobi - jacobi_min) / (jacobi_max - jacobi_min) if jacobi_max != jacobi_min else 0.5
        )
