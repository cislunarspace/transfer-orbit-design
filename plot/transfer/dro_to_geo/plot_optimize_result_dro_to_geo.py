...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m plot.transfer.dro_to_geo.plot_optimize_result_dro_to_geo --help
"""
...
from plot.config import apply_standard_plot_config  # noqa: E402
...
def _plot_orbit_2d(
    ...
    fig.tight_layout()
    if caption:
        fig.text(0.5, -0.02, caption, ha="center", va="top", fontsize=PLOT_CONFIG.tick)
    if save_path:
        png = Path(save_path)
...
def main() -> None:
    ...
    parser.add_argument(
        "--auto-latest",
        action="store_true",
        help="显式 opt-in：按 mtime 选最新 optimization_dro_geo_*.json",
    )
    ...
    parser.add_argument(
        "--auto-latest-dro", action="store_true", help="显式 opt-in：按 mtime 选最新 dro_*.json"
    )
    parser.add_argument(
        "--figsize", type=str, default=None, help="图尺寸（厘米），格式 '宽,高'，如 '8.5,8.5'"
    )
    ...
    opt_path = resolve_opt_input(
        args, pattern="optimization_dro_geo_*.json", prog="plot_optimize_result_dro_to_geo"
    )
    ...
    if args.orbit:
        ...
        figsize = figsize_cm or ((12 / 2.54, 8 / 2.54) if args.paper else (10, 7))
        ...
        if len(selected) == 1:
            ...
            try:
                ...
                ax.scatter(*dep_state[:3], color="green", s=40, zorder=5, label="出发点")
                ax.scatter(
                    *transfer_states[-1, :3],
                    color="orange",
                    s=40,
                ...
                if args.caption:
                    fig.text(
                        0.5, -0.02, args.caption, ha="center", va="top", fontsize=PLOT_CONFIG.tick
                    )
            except Exception:
                plt.close(fig)
                raise
        ...
        figsize_cm = _resolve_figsize_cm(args.figsize)
        figsize = figsize_cm or ((12 / 2.54, 8 / 2.54) if args.paper else None)
