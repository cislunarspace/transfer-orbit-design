...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m plot.transfer.dro_to_ro.plot_search_results_dro_to_ro --help
"""
...
RESULTS_JSON = project_root / (
    "output/transfer/search_results_200-100-0.5-2.5-22.998482_3857848453.json"
)
# 示例:
# 示例: 替换为实际文件名，格式为 search_results_xxx.json
...
def _plot_single_transfer_orbit(
    ...
    all_pts = np.concatenate(
        [transfer_states[:, :3], departure_orbit.states[:, :3], arrival_orbit.states[:, :3]]
    )
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default="0",
        help=(
            "选择可行解：整数索引，'best'（Δv 最小 1 个），"
            "'best:N'（Δv 最小的 N 个），'random'，"
            "或 'all'（全部，受 --max-points 控制）"
        ),
    )
    parser.add_argument(
        "--n-workers",
