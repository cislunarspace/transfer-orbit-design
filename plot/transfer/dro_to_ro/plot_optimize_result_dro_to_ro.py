...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m plot.transfer.dro_to_ro.plot_optimize_result_dro_to_ro --help
"""
...
from plot.transfer.common import resolve_opt_input
from src.commons.constants import MU, TU, VU
from src.commons.orbits import EARTH_CENTER
from src.commons.orbits import compute_departure_velocity as _compute_departure_velocity
...
def _select_indices(
    records: list[dict[str, Any]], idx_arg: str, seed: int, max_points: int | None = None
) -> list[int]:
    n = len(records)
...
def plot_orbit_3d(
    ...
    if n_sel == 1:
        ...
        # 等比例轴
        all_pts = np.concatenate([states[:, :3], dro_orbit.states[:, :3], ro_orbit.states[:, :3]])
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="可视化 NLP 优化结果", formatter_class=argparse.ArgumentDefaultsHelpFormatter
    )
    parser.add_argument(
        "--file", type=str, default=None, help="optimization_results_*.json 路径（默认自动选最新）"
    )
    parser.add_argument(
        "--auto-latest",
        action="store_true",
        help="显式 opt-in：按 mtime 选最新 optimization_results_*.json",
    )
    ...
    parser.add_argument(
        "--idx", type=str, default="best", help="选择结果：整数索引 / best / best:N / random / all"
    )
    parser.add_argument("--seed", type=int, default=0)
    parser.add_argument(
        "--max-points", type=int, default=500, help="--orbit --idx all 时最多绘制条数"
    )
    args = parser.parse_args()

    opt_path = resolve_opt_input(
        args, pattern="optimization_results_*.json", prog="plot_optimize_result_dro_to_ro"
    )
    ...
    if args.orbit:
        ...
        plot_orbit_3d(
            records, sel_indices, dro_orbit, ro_orbit, system, dynamics, save_path, args.dpi
        )
    elif args.time_dv:
