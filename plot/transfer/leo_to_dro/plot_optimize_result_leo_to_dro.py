"""plot_optimize_result_leo_to_dro 可视化脚本。
...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m plot.transfer.leo_to_dro.plot_optimize_result_leo_to_dro --help
"""
...
def _successful_records(results: list[dict], max_pos_err_km: float) -> list[dict]:
    ...
    for row in results:
        nlp = row.get("nlp", {})
        if not nlp.get("success", False):
            continue
        pos_violation = nlp.get("pos_violation")
        if (
            pos_violation is not None
            and np.sqrt(max(0, float(pos_violation))) * DU > max_pos_err_km
        ):
            continue
        success.append(row)
    return success
...
def _interactive_browse(records: list[dict]) -> None:
    ...
    if not selected:
        logger.info("无成功结果可浏览")
        return
    logger.info("交互浏览：Enter=下一条, q=退出")
    for i, row in enumerate(selected, start=1):
        nlp = row.get("nlp", {})
        logger.info(
            f"[{i}/{len(selected)}] T={nlp.get('transfer_time', 0) * TU:.2f} 天, "
            f"Δv={nlp.get('objective_value', 0) * VU / 1000:.6f} km/s, "
            f"alpha={nlp.get('alpha', 'N/A')}"
        )
        try:
...
def main() -> None:
    ...
    parser.add_argument(
        "--auto-latest",
        action="store_true",
        help="显式 opt-in：按 mtime 选最新 optimization_leo_dro_*.json",
    )
    ...
    parser.add_argument("--idx", type=str, default="best:5", help="选择索引")
    parser.add_argument("--max-pos-err", type=float, default=100.0, help="最大位置误差 (km)")
    parser.add_argument("--save", type=str, default=None, help="保存图片路径")
    parser.add_argument("--max-points", type=int, default=200, help="最大绘制轨道数")
    ...
    opt_path = resolve_opt_input(
        args, pattern="optimization_leo_dro_*.json", prog="plot_optimize_result_leo_to_dro"
    )
    ...
    success = _successful_records(results, args.max_pos_err)
    logger.info(f"结果总数: {len(results)}, 成功且满足过滤: {len(success)}")

    if args.interactive:
        _interactive_browse(success)
        return
