...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m plot.transfer.leo_to_dro.plot_search_results_leo_to_dro --help
"""
...
def _selected_feasible(
    feasible: list[dict], idx_arg: str, seed: int, max_points: int
) -> list[dict]:
...
def main() -> None:
    ...
    parser.add_argument(
        "--auto-latest", action="store_true", help="显式 opt-in：按 mtime 选最新搜索结果 JSON"
    )
