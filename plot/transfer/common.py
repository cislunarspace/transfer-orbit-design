...
from plot._io_utils import load_search_results  # noqa: F401 — 重导出供各 plot 脚本统一使用
from src.commons.orbits import (
    compute_departure_velocity,  # noqa: F401 — 重导出供各 plot 脚本统一使用
)
...
def plot_alpha_delta_v(
    ax: Axes, alpha: np.ndarray, delta_v: np.ndarray, title_prefix: str, *, config=None
) -> None:
    """绘制 α vs Δv_departure 散点图。"""
    from src.commons.constants import VU
...
def plot_transfer_time_delta_v(
    ...
    *,
    ...
    """
    from plot.config import style_colorbar
    from src.commons.constants import TU, VU
...
def geo_circle_points(n_pts: int = 200) -> tuple[np.ndarray, np.ndarray]:
    """GEO 球面在 x-y 平面上的投影圆。"""
    from src.commons.orbits import EARTH_CENTER, R_GEO
...
def plot_celestial_bodies(ax, system, config) -> None:
    ...
    ax.text(
        EARTH_CENTER[0], EARTH_CENTER[1] + 0.03, 0, "地球", fontsize=config.lp_label, ha="center"
    )
...
def set_equal_aspect_3d(ax, points: np.ndarray) -> None:
    ...
    z_half = ptp[:2].max() * 0.05 + 0.05 if ptp[2] < 1e-08 and ptp[:2].max() > 1e-08 else half
...
def build_transfer_dynamics(mu: float | None = None, dt: float | None = None):
    ...
    from src.commons.constants import MU, TU
...
def _annotate_bodies_2d(ax, system, config) -> None:
    ...
    ax.text(EARTH_CENTER[0], EARTH_CENTER[1] + 0.03, "地球", fontsize=config.lp_label, ha="center")
    ax.text(1.0 - MU, 0.03, "月球", fontsize=config.lp_label, ha="center")
    ...
    for lp_name, lp_x in [("L1", system.L1[0]), ("L2", system.L2[0])]:
        ...
        ax.text(lp_x, 0.02, lp_name, fontsize=config.lp_label, ha="center", color="red")
...
def plot_single_transfer_orbit_2d(
    ...
    *,
    ...
    ax.plot(transfer_states[:, 0], transfer_states[:, 1], color="crimson", lw=1.2, label="转移轨道")
