...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m plot.transfer.geo_to_dro.plot_optimize_result_geo_to_dro --help
"""
...
from src.commons.constants import DU, TU, VU
from src.commons.input_contract import (
    InputFileRequest,
...
def plot_transfer_time_vs_dv(records, ax):
    ...
    ax.annotate(
        (
            ...
            f"Δv={best['objective_value'] * VU / 1000:.3f} km/s"
        ),
...
def plot_orbit_3d(records, sel_indices, dro_orbit, system, dynamics, save_path=None, dpi=150):
    ...
    if len(sel_records) == 1:
        ...
        # 转移轨道
        ax.plot(states[:, 0], states[:, 1], states[:, 2], color="crimson", lw=1.2, label="转移轨道")
        ...
        sm = plt.cm.ScalarMappable(
            cmap=cmap, norm=Normalize(vmin=obj_min * VU / 1000, vmax=obj_max * VU / 1000)
        )
        sm.set_array([])
...
def interactive_browse(records, dro_orbit, system, dynamics, max_pos_err_km=100.0):
    ...
    while 0 <= current < n:
        ...
        logger.info(
            f"\n[{current + 1}/{n}] (idx={orig_i}) a={alpha:.4f}, "
            f"T={tt * TU:.1f} 天, "
            f"dv1={dv1 * VU / 1000:.4f}, dv2={dv2 * VU / 1000:.4f}, "
            f"total={total * VU / 1000:.4f} km/s, "
...
def main():
    ...
    parser.add_argument(
        "--file", type=str, default=None, help="optimization JSON path (auto-detect latest)"
    )
    parser.add_argument(
        "--auto-latest",
        action="store_true",
        help="显式 opt-in：按 mtime 选最新 optimization_geo_dro_*.json",
    )
    parser.add_argument("--dro-file", type=str, default=None, help="DRO 轨道 JSON 路径")
    parser.add_argument(
        "--auto-latest-dro",
        action="store_true",
        help="显式 opt-in：按 mtime 选最新 dro_<digits>.json",
    )
    ...
    parser.add_argument("--max-points", type=int, default=200, help="max orbits for --idx all")
    parser.add_argument(
        "--max-pos-err", type=float, default=100.0, help="max position error (km) to include"
    )
    parser.add_argument("--save", type=str, default=None, help="save figure to path")
    parser.add_argument("--dpi", type=int, default=150)
    args = parser.parse_args()

    opt_path = resolve_opt_input(
        args, pattern="optimization_geo_dro_*.json", prog="plot_optimize_result_geo_to_dro"
    )
    ...
    n_valid = sum(
        ...
        for r in records
        if r["success"]
        and np.sqrt(max(0, float(r.get("pos_violation", 1e10)))) * DU < args.max_pos_err
    )
    logger.info(
        f"Total: {n_total}, success: {n_success}, valid (pos<{args.max_pos_err:.0f}km): {n_valid}"
    )
    ...
    if args.interactive:
        ...
        indices = _select_indices(
            ...
            max_pos_err_km=args.max_pos_err,
        )
        logger.info(f"Plotting {len(indices)} orbits (idx={args.idx})")
        plot_orbit_3d(
            records, indices, dro_orbit, system, dynamics, save_path=args.save, dpi=args.dpi
        )
