...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m plot.transfer.geo_to_dro.plot_search_results_geo_to_dro --help
"""
...
def _plot_single_transfer_orbit(
    ...
    geo_orbit.states if hasattr(geo_orbit, "states") else geo_orbit
    # GEO 圆
...
def interactive_browse_by_time(feasible_rows, dro_orbit, system, dynamics):
    ...
    while 0 <= current < n:
        ...
        try:
            transfer_states, times = _reintegrate_transfer(
                dynamics, dep_state, alpha, tt, dro_orbit=dro_orbit
            )
        ...
        ax.scatter(
            *transfer_states[-1, :3], color="orange", s=40, marker="s", zorder=5, label="Arrival"
        )
        # 天体和平动点
        plot_celestial_bodies(ax, system, PLOT_CONFIG)
...
def main():
    ...
    parser.add_argument(
        "--auto-latest",
        action="store_true",
        help="显式 opt-in：按 mtime 选最新 search_geo_dro_*.json",
    )
    ...
    parser.add_argument(
        "--max-points", type=int, default=50000, help="散点最多绘制的可行点数（过多时随机子采样）"
    )
    ...
    parser.add_argument(
        "--idx", type=str, default="best:10", help="轨道选择: all, best, best:N, random, 或序号"
    )
    ...
    if args.interactive:
        ...
        indices = select_feasible_indices(
            feasible_rows, args.idx, seed=args.seed, max_indices=args.max_points
        )
