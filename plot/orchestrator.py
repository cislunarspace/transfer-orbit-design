...
本模块是 `FamilyPlotOrchestrator` 及其配置 dataclass 的实现，被 `plot.plot_orbits`
...
运行示例（通过入口脚本调用）:
    .. code-block:: bash
       ...
       uv run python -m plot.plot_orbits --help
"""
...
from plot._plot_2d import _plot_bodies_and_libration, _project_2d  # noqa: F401
from plot._plot_3d import (  # noqa: F401
    _get_center_coordinates,
    _resolve_3d_center_radius,
    compute_view_bounds,
)
from plot._stability import compute_stability_indices
...
class FamilyPlotOrchestrator:
    ...
    def _run_single(
        ...
        if want_2d:
            self._render_2d(
                plotter, subset, jacobi_subset, bounds, output_dir, family_name, n_orbits, step
            )
        if want_3d:
            self._render_3d(
                plotter, subset, jacobi_subset, bounds, output_dir, family_name, n_orbits, step
    ...
    def _run_multi(
        ...
        for i, config in enumerate(multi_configs):
            ...
            logger.info(
                f"[{i + 1}/{len(multi_configs)}] 加载 {family_path.stem}: {len(subset)} 条轨道"
            )
        ...
        for subset, jacobi in zip(all_subsets, all_jacobi, strict=False):
        ...
        logger.info(
            f"合并后共 {total_orbits} 条轨道，"
            f"Jacobi 范围: {min(merged_jacobi):.6f} ~ {max(merged_jacobi):.6f}"
        )
    ...
    def _render_2d(
        ...
        kwargs["title"] = (
            f"{cfg.display_name.upper()} Orbit Family ({cfg.plane.upper()} Plane) "
            f"- {n_orbits} orbits\nC = [{jmin:.4f}, {jmax:.4f}]"
        )
    ...
    def _render_2d_multi(
        ...
        # 设置标题
        ax.set_title(
            f"{cfg.display_name.upper()} Orbit Families ({plane.upper()} Plane) "
            f"- {total_orbits} orbits\nC = [{jmin:.4f}, {jmax:.4f}]"
        )
        ...
        cmap = matplotlib.colormaps[
            "viridis"
        ]  # 新版 matplotlib 删除了 cm.get_cmap（CI pyright 报此错）

        with warnings.catch_warnings():
            warnings.filterwarnings("ignore", message=".*Tight layout.*")

            # 为每个 family 分别绘制到同一个坐标轴（使用各自 step）
            for subset, jacobi, step in zip(subsets, jacobis, steps, strict=False):
    ...
    def _render_jacobi_stability(
        ...
        kwargs: dict = dict(
            title=(
                f"{cfg.display_name.upper()} Orbit Family - Period and Stability (n = {n_orbits})"
            ),
