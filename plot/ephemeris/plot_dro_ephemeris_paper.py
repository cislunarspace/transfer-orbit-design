...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m plot.ephemeris.plot_dro_ephemeris_paper --help
"""
...
EPHEMERIS_JSON_DEFAULT = (
    project_root / "output" / "ephemeris" / "dro_single_ephemeris_conversion_20260609_105936.json"
)
...
def main(argv=None):
    ...
    out_file = (
        Path(args.out_file)
        if args.out_file
        else (project_root / "figures" / "结果" / "星历修正DRO论文版.png")
    )
    ...
    try:
        ...
        # 平均月距
        r_moon_syn = np.linalg.norm(eph_full_syn[:, :3] - np.array([1 - MU, 0, 0]), axis=1) * DU
        ...
        ax1.plot(
            eph_full_j2000[:, 0] / 1e3,
            eph_full_j2000[:, 1] / 1e3,
            "-",
            color=COLOR_EPH,
            lw=1.5,
            label="星历修正",
        ...
        ax1.scatter(
            corrected_states[:, 0] / 1e3,
            corrected_states[:, 1] / 1e3,
            c=COLOR_PATCH,
            s=18,
            zorder=5,
            marker="o",
