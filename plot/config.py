...
colorbar 样式与轨道采样等配置，供 ``plot`` 下的可视化脚本 import 使用，
本身不可作为脚本运行。
"""
...
# 向后兼容重导出：所有符号仍可从 plot.config 导入
from plot._font_config import (
    BODY_ICON_SCALE_ENV,
    PLOT_FONT_ENV_VARS,
    PLOT_FONT_KEYS,
...
from plot._subsample import subsample_indices
...
def style_colorbar(colorbar, config, label: str | None = None):
    ...
    if label is not None:
        ...
        colorbar.set_label(
            colorbar.ax.get_ylabel() or colorbar.ax.get_xlabel(), fontsize=config.colorbar
        )
