...
def CR3BP_Newton_Differential_Correction_3D_Symmetric_XZ_Fixed_X0(
    SV0_initial, tf_initial, tol, N, maxiter
):
    """
    定义微分修正方法，利用牛顿迭代法修正关于xz平面对称的周期轨道初值，固定x0，修正z0,y0_dot和周期T（适用于Halo轨道，Vertical轨道）
    ...
    # 定义修正变量为z0, y0_dot, tf_half,将待修正的位置速度初值和轨道周期的一半代入
    ...
    while True:
        ...
        # 定义微分校正系数矩阵, dF = [[phi_23, phi_25, y_dot],[phi_43, phi_45, x_ddot],[phi_63, phi_65, z_ddot]]
        dX_Constraint = np.empty((3, 3))

        dX_Constraint[0, 0] = CR3BP_STM[1, 2]
        dX_Constraint[0, 1] = CR3BP_STM[1, 4]
        dX_Constraint[0, 2] = CR3BP_dot_SV[1]

        dX_Constraint[1, 0] = CR3BP_STM[3, 2]
        dX_Constraint[1, 1] = CR3BP_STM[3, 4]
        dX_Constraint[1, 2] = CR3BP_dot_SV[3]

        dX_Constraint[2, 0] = CR3BP_STM[5, 2]
        dX_Constraint[2, 1] = CR3BP_STM[5, 4]
        dX_Constraint[2, 2] = CR3BP_dot_SV[5]

        dX_Constraint = np.linalg.inv(dX_Constraint)

        # 定义变量z0, y0_dot, tf_half的修正量
