...
def CR3BP_Newton_Differential_Correction_2D_Symmetric_X_Fixed_T(
    SV0_initial, tf_initial, tol, N, maxiter
):
    """
    定义微分修正方法，利用牛顿迭代法修正关于x轴对称的平面周期轨道初值，固定周期T，修正x0和y0_dot（适用于Lyapunov轨道，DRO轨道）
    ...
    # 定义修正变量为x0, y0_dot,将待修正的位置速度初值和轨道周期的一半代入
    SV0 = copy.deepcopy(SV0_initial)
    tf = tf_initial
    tf_half = 0.5 * tf

    X_Variable = np.empty(2)
    ...
    while True:
        ...
        # 定义微分校正系数矩阵, dF = [[phi_21, phi_25],[phi_41, phi_45]]
        dX_Constraint = np.empty((2, 2))
