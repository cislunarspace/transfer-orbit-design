...
from Dynamics.CR3BP_Dynamics import CR3BP_Dynamics, CR3BP_Propagation_42


def CR3BP_Newton_Differential_Correction_2D_Symmetric_X_Correct_All(
    SV0_initial, tf_initial, tol, N, maxiter
):
    """
    定义微分修正方法，利用牛顿迭代法修正关于x轴对称的平面周期轨道初值，同时修正x0，y0_dot和周期T（适用于Lyapunov轨道，DRO轨道）
        输入为待修正的轨道位置速度向量初值与周期的猜测值，容差，完整周期的离散点个数，最大迭代次数。
        输出为修正后的轨道位置速度向量初值与周期。
        轨道未修正初值取为 SV0 = [x0  0  0  0  y0_dot 0],待修正周期为tf，
        积分tf/2后，若轨道状态第一次回到x轴上(即y=0)，且x_dot=0，则得到轨道的一半，计算完毕；
        否则对初值SV0和周期T进行修正。
    """

    # 定义修正变量为x0, y0_dot, tf_half,将待修正的位置速度初值和轨道周期的一半代入
    ...
    while True:
        ...
        # 定义约束条件向量为积分终点的y, x_dot
        X_Constraint = np.empty(2)
        ...
        # 定义微分校正系数矩阵, dF = [[phi_21, phi_25, y_dot],[phi_41, phi_45, x_ddot]]
        dX_Constraint = np.empty((2, 3))

        dX_Constraint[0, 0] = CR3BP_STM[1, 0]
        dX_Constraint[0, 1] = CR3BP_STM[1, 4]
        dX_Constraint[0, 2] = CR3BP_dot_SV[1]

        dX_Constraint[1, 0] = CR3BP_STM[3, 0]
        dX_Constraint[1, 1] = CR3BP_STM[3, 4]
        dX_Constraint[1, 2] = CR3BP_dot_SV[3]

        dX_Constraint = np.matmul(
            dX_Constraint.T, np.linalg.inv(np.matmul(dX_Constraint, dX_Constraint.T))
        )

        # 定义变量x0, y0_dot, tf_half的修正量
        ...
        if np.linalg.norm(X_Constraint - np.zeros(2)) < tol:
    ...
    SV0[4] = X_Variable[1]
    SV0_corrected = SV0
    tf_half_corrected = X_Variable[2]
