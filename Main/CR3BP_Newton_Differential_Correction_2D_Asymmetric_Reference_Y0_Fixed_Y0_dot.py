...
def CR3BP_Newton_Differential_Correction_2D_Asymmetric_Reference_Y0_Fixed_Y0_dot(
    SV0_initial, tf_initial, tol, N, maxiter
):
    """
    定义非对称微分修正方法，利用牛顿迭代法修正周期轨道初值，固定y0_dot，修正x0,y0和x0_dot（适用于三角平动点附近的平面周期轨道）
          输入为待修正的轨道位置速度向量初值与周期的猜测值，容差，完整周期的离散点个数，最大迭代次数。
          输出为修正后的轨道位置速度向量初值与周期。
          轨道未修正初值取为 SV0 = [x0  y0  0  x0_dot  y0_dot 0],待修正周期为tf。
          修正量形式为 delta_SV0 = [delta_x0  delta_y0  delta_x0_dot]
    """

    # 定义修正变量为x0, x0_dot, 将待修正的位置速度初值和轨道周期代入
    ...
    while True:
        ...
        # 定义约束条件向量为积分终点的delta_x,delta_x_dot
        X_Constraint = np.empty(2)

        X_Constraint[0] = CR3BP_SV[0] - SV0[0]
        X_Constraint[1] = CR3BP_SV[3] - SV0[3]

        print("当前约束变量" + str(X_Constraint))

        # 计算Q矩阵
        # 调整后的状态转移矩阵
        CR3BP_STM_1 = np.delete(CR3BP_STM, [1, 2, 4, 5], axis=0)
        CR3BP_STM_1 = np.delete(CR3BP_STM_1, [1, 2, 5], axis=1)
        CR3BP_STM_2 = CR3BP_STM[1, :]
        CR3BP_STM_2 = np.delete(CR3BP_STM_2, [1, 2, 5])

        # 调整后的状态向量一阶导,第一部分为y_dot, 第二部分为x_dot,x_ddot
        CR3BP_dot_SV_1 = CR3BP_dot_SV[1]
        CR3BP_dot_SV_2 = np.delete(CR3BP_dot_SV, [1, 2, 4, 5])
