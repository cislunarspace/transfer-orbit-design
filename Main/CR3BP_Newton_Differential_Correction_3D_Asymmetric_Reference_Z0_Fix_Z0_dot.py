...
def CR3BP_Newton_Differential_Correction_3D_Asymmetric_Reference_Z0_Fix_Z0_dot(
    SV0_initial, tf_initial, tol, N, maxiter
):
    """
    定义非对称微分修正方法，利用牛顿迭代法修正周期轨道初值，固定y0_dot，修正x0,y0和x0_dot（适用于L4,L5点Axial轨道）
          ...
          修正量形式为 delta_SV0 = [delta_x0  delta_y0  delta_x0_dot  delta_y0_dot  delta_z0_dot]
    """

    # 定义修正变量为x0, y0, x0_dot, y0_dot, z0_dot,将待修正的位置速度初值和轨道周期代入
    ...
    while True:
        ...
        # 定义约束条件向量为积分终点的delta_x,delta_y,delta_x_dot,delta_y_dot
        X_Constraint = np.empty(4)

        X_Constraint[0] = CR3BP_SV[0] - SV0[0]
        X_Constraint[1] = CR3BP_SV[1] - SV0[1]
        X_Constraint[2] = CR3BP_SV[3] - SV0[3]
        X_Constraint[3] = CR3BP_SV[4] - SV0[4]

        print("当前约束变量" + str(X_Constraint))

        # 计算P矩阵
        # 调整后的状态转移矩阵（x,y,x_dot,y_dot对除z0的其他元素的偏导）
        CR3BP_STM_1 = np.delete(CR3BP_STM, [2, 5], axis=0)
        CR3BP_STM_1 = np.delete(CR3BP_STM_1, 2, axis=1)
        CR3BP_STM_2 = CR3BP_STM[2, :]
        CR3BP_STM_2 = np.delete(CR3BP_STM_2, 2)

        # 调整后的状态向量一阶导,第一部分为y_dot, 第二部分为x_dot,z_dot,x_ddot,z_ddot
        CR3BP_dot_SV_1 = CR3BP_dot_SV[2]
        CR3BP_dot_SV_2 = np.delete(CR3BP_dot_SV, [2, 5])

        P = CR3BP_STM_1 - 1 / CR3BP_dot_SV_1 * np.outer(CR3BP_dot_SV_2, CR3BP_STM_2.T)

        # 定义微分校正系数矩阵
        dX_Constraint = P

        dX_Constraint[0, 0] = P[0, 0] - 1
        dX_Constraint[1, 1] = P[1, 1] - 1
        dX_Constraint[2, 2] = P[2, 2] - 1
        dX_Constraint[3, 3] = P[3, 3] - 1
