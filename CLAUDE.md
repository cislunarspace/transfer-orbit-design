...
本仓库的 issue 和 PRD 存放在 GitHub Issues 中；外部 PR 也进入分诊流程。所有操作使用 `gh` CLI。见 `docs/agents/issue-tracker.md`。

### Triage labels

五个标准分诊角色，标签名与角色名相同（`needs-triage`、`needs-info`、`ready-for-agent`、`ready-for-human`、`wontfix`）。见 `docs/agents/triage-labels.md`。

### Domain docs

单一上下文布局：根目录 `CONTEXT.md` + `docs/adr/`。见 `docs/agents/domain.md`。

### Loop Engineering

`/loop-go <task>` 循环运行 builder（写/修代码）和 checker（跑全部检查）直到通过。三个文件：`.claude/agents/builder.md`、`.claude/agents/checker.md`、`.claude/commands/loop-go.md`。
