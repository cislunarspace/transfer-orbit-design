...
**注意**：初始状态为 `[x₀, 0, 0, 0, vy₀, 0]`（x 轴上的交点，y=0），使用 `setup_2D_symmetric_x_fixed_x0` 模式修正。
...
```
约束条件：
- x₀ 固定（使用 setup_2D_symmetric_x_fixed_x0 模式）
- y(0) = 0（初始在 x 轴上）
- y(T/2) = 0 — 半周期时穿过 x 轴
- vx(T/2) = 0 — 垂直穿过
...
```bash
python -m tod.pipelines.ro.generate_31_ro_family
```
...
```bash
python -m tod.pipelines.ro.generate_32_ro_family
```
...
```bash
# 可视化 3:1 RO
python -m tod.pipelines.ro.plot_31_ro_family

# 可视化 3:2 RO
python -m tod.pipelines.ro.plot_32_ro_family
```
