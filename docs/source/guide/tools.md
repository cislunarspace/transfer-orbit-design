...
不变流形分析 / 低能转移 / 相对运动）灰显，悬停显示工具说明（区分"GUI 尚未接入"
与"e2m2e 占位"）。
...
| `correction_method` | two_level | 星历修正方法：稳定轨道公开选项为 `two_level`（e2m2e 5.8.0 起它就是 Rust 多重打靶的对外名称，不再与 `standard` 区分）。不稳定轨道无需手选：Halo/NRHO/DPO 由 e2m2e 自动改走 `segmented`，Lissajous 由 GUI 桥接层同样强制 `segmented`（见 {doc}`../concepts/ephemeris`） |
...
| `special_mode` | 自动识别 | 按选中轨道设置：Halo/NRHO 为 2（ẋ=0 且 ż=0），其余轨道为 1（Lissajous，ẋ=0） |
| `control_interval` | 0.25 天 | 控制时间间隔（可切 天/秒/TU） |
| `feedback_arc` | 0.125 天 | 目标点模式反馈弧段（可切 天/秒/TU） |
| `num_controls` | 120 | 控制次数 |
...
运行前校验：**目标点模式的最长取样时刻（= (控制次数-2) × 控制间隔 +
反馈弧）不得超过标称星历覆盖时长**，超出会拦截并提示调整参数或延长标称轨道。
GUI 默认的 0.25 天/次、0.125 天反馈弧在 120 次控制下覆盖约 29.6 天，适合
GUI 默认设计的短弧；较长任务可按星历覆盖时长调整参数。
