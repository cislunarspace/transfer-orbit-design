...
GUI 提供四个工具。参数面板由工具绑定的 Pydantic 模型自动生成，字段范围、
默认值与说明来自模型本身；本节列出各工具的常用参数。单位可切换的字段
（振幅、近月点高度、时长、输出步长）在面板中有单位下拉框，切换单位只改变
显示数值，不改变物理量。

## 轨道设计（design_orbit）

在 CR3BP 中生成周期轨道并修正到星历模型。可选轨道类型：
**DRO / Halo / NRHO / Lissajous / L4 / L5 / ELFO**。选择类型后面板只显示
该类型相关的形状参数。
...
| 轨道类型 | 参数 | 说明 |
|----------|------|------|
| DRO | `amplitude` | 距月振幅（km，默认 60000；可切 DU） |
| DRO | `phase` | 相位（默认 0.5001） |
| Halo | `collinear_point` | 共线平动点（1=L1，2=L2，默认 2） |
| Halo | `amplitude` | 面外振幅（km，默认 30000） |
| Halo | `phase` | 相位（默认 0） |
| NRHO | `collinear_point` | 共线平动点（默认 2） |
| NRHO | `north_south` | 北族（1）/ 南族（2），默认 2 |
| NRHO | `perilune_height` | 近月点高度（km，默认 5000） |
| NRHO | `phase` | 相位（默认 0.5） |
| Lissajous | `collinear_point` | 共线平动点（默认 2） |
...
| ELFO | `inclination` / `arg_of_pericenter` | 倾角 / 近月点幅角（度，默认 75 / 270） |
...
| `duration` | 1 个月 | 传播时长（默认 1 月；可切 年/月/日/TU。轨道保持以它为输入，建议 ≥ 保持总时长） |
| `output_step` | 3600 s | 星历输出步长（可切 秒/TU） |
...
| `perturbation` / `dyb` | 空 | 可选摄动项（太阳引力等）/ DYB 摄动开关 |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `control_mode` | 1 | 控制模式：1=目标点宽松、2=目标点严格、3=特征点；4/5/6 为对应模式 + 角动量管理 |
| `is_nrho` | 0 | 目标轨道是否 NRHO |
| `special_mode` | 1 | 特征点模式：1=Lissajous（ẋ=0）、2=Halo/NRHO（ẋ=0 且 ż=0） |
| `control_interval` | 30 天 | 控制时间间隔（天） |
| `feedback_arc` | 28 天 | 目标点模式反馈弧段（天） |
| `num_controls` | 120 | 控制次数（总时长 = (次数-1) × 间隔） |
| `num_monte_carlo` | 5 | 蒙特卡洛样本数 |
| `output_step` | 86400 s | 受控星历输出间隔 |
| `engine_layout` | 空 | 发动机布局 JSON（`positions_m` / `directions`），角动量管理（模式 4–6）必填 |
| `momentum_interval` | 5 天 | 角动量卸载间隔，0 = 与轨道控制同步 |
| `spacecraft_mass` | 1000 kg | 航天器质量 |
| `srp_offset_m` | 空 | SRP 压心相对质心偏移 [x,y,z]（m） |
...
| 参数 | 默认值 | 说明 |
|------|--------|------|
| `libration_point` | 2 | 共线平动点（1=L1，2=L2） |
| `max_amplitude_km` | 30000 | 最大面外振幅（km，范围 1000–57000）；延拓到该振幅或折叠点自动停止 |
