...
- **工具面板**：七个工具全部接入：轨道族生成、任务轨道设计、轨道保持、轨道预报、转移轨道设计、轨道稳定性、时空坐标转换。参数表单由各工具的 JSON Schema 自动生成（字段裁剪、范围与默认值随 e2m2e 模型走），经通用工具执行通道（Rust `run_tool` 命令）直达 sidecar，错误直显；执行前防呆校验（必填与数值范围，内联标红）。
