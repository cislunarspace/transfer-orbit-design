...
Tests for FacadeBridge.generate_family / analyze_stability (family generation + stability analysis).
...
Mock tests verify DTO assembly and parameter passthrough; the real-path
tests at the end use real e2m2e (pure CR3BP, no SPICE kernels) to verify
family generation actually runs. Since 5.7.1 generation goes through
``Facade.orbit_family_generation`` (the unified entry for all seven
families), so mocks stub the Facade method itself.
"""
...
class TestFamilyGenerationRequest:
    ...
    def test_seven_families_accepted(self):
        ...
        Since 5.7.1 all seven families are legal orbit_types
        (libration points and parameter defaults filled per family).
        """
    ...
    def test_cross_family_fields_rejected(self):
        ...
        Since 5.7.1 cross-family fields are rejected per model_fields_set
        (None counts as set).
        """
        with pytest.raises(ValidationError):
    ...
    def test_dro_family_defaults(self):
        ...
        Since 5.8.2 DRO (Moon-centered) is a legal orbit_type: no libration point binding,
        amplitude range defaulting to 2000-60000 km.
        """
...
# generate_family mock tests (stub on Facade.orbit_family_generation)
# ---------------------------------------------------------------------------


class _FakeSystem:
    """Fake CR3BP_System。A fake CR3BP_System."""
...
class _FakeDynamics:
    ...
    Fake CR3BP_Dynamics: propagate returns equal-length trajectories tiled
...
def _fake_response(
    ...
    *,
    ...
    Build a fake response shaped like FamilyGenerationResponse (members carry
    full trajectories by default).
    """
...
def mock_family_facade(monkeypatch):
    ...
    Stub out Facade.orbit_family_generation and record the bridge's arguments.
    """
...
class TestGenerateFamily:
    ...
    def test_none_params_stripped(self, mock_family_facade):
        ...
        None values (unchecked Optionals) are stripped before reaching the Facade,
        avoiding cross-family field rejection.
        """
    ...
    def test_periodic_members_resampled(self, monkeypatch):
        ...
        When periodic members carry only an initial state and a period, the bridge
        resamples the full trajectory per period.
        """
        from e2m2e.data.templates import ConvergenceState, FailureCause
    ...
    def test_dro_family_libration_point_none(self, monkeypatch):
        ...
        Moon-centered DRO members carry no libration_point parameter: the DTO field
        is None while the display name still maps normally.
        """
    ...
    def test_invalid_params_translated(self):
        ...
        Invalid arguments hit the real Facade's FamilyGenerationRequest validation,
        surfacing as OrbitError(INVALID_PARAMS).
        """
        from src.engine.exceptions import OrbitError
    ...
    def test_cross_family_params_translated(self):
        ...
        Cross-family fields (north_south on Halo) are rejected by the 5.7.1 model
        → INVALID_PARAMS.
        """
        from src.engine.exceptions import OrbitError
...
# analyze_stability mock tests
# ---------------------------------------------------------------------------


class _FakeStabilityAnalysis:
    """Fake StabilityAnalysis：analyze() 返回纯数据结果容器。
    Fake StabilityAnalysis: analyze() returns a pure-data result container.
    """
...
class TestAnalyzeStability:
    ...
    def test_receives_orbit_with_system(self, mock_stability):
        ...
        The constructed Orbit must be bound to a system (analyze takes mu from
    ...
    def test_mu_none_uses_earth_moon_default(self, mock_stability):
        ...
        When mu is missing (legacy Artifact), the e2m2e Earth-Moon default mass ratio
        is used instead of a hard-coded constant.
        """
        FacadeBridge().analyze_stability(
            states=np.random.randn(10, 6),
            times=np.linspace(0, 1, 10),
...
# Real path (pure CR3BP, no SPICE needed)
# ---------------------------------------------------------------------------


def test_generate_family_real_pipeline(tmp_path):
    """真 e2m2e：小 Halo 族应返回等长三维 states（周期族成员由桥接层按周期重采样）。
    Real e2m2e: a small Halo family must return equal-length 3-D states (periodic
    members resampled per period by the bridge).

    Since 5.8.0 families auto-ingest (one record per family); record_id is
    ...
    # Northern-family z amplitude increases monotonically (continuation from
    # the seed toward larger amplitudes)
    assert data.z0s[0] < data.z0s[-1]
    # 产物入库：一族一条记录，成员参数在记录内（issue #375 US3/US8）
    # Products ingest into the library: one record per family, member parameters
    # inside the record (issue #375 US3/US8)
...
def test_generate_nrho_family_real_pipeline():
    ...
    Real e2m2e: the NRHO family (dispatch added in 5.7.1) must return members
    carrying perilune altitude.
    """
    data = FacadeBridge().generate_family(
...
def test_generate_lissajous_family_real_pipeline():
    ...
    Real e2m2e: the Lissajous family is quasi-periodic parameter sampling; members
    come with equal-length full trajectories.
    """
    data = FacadeBridge().generate_family(
    ...
    # Sample fractions 1/3, 2/3, 1: out-of-plane amplitude bounds increase linearly
...
def test_generate_dro_family_real_pipeline(tmp_path):
    ...
    Real e2m2e: the DRO family (5.8.2, #502) is Moon-centered with no libration
    point; member parameters are amplitude_km.
    """
    bridge = FacadeBridge(catalog_dir=str(tmp_path / "catalog"))
...
def test_analyze_stability_real_pipeline():
    ...
    Real e2m2e: stability analysis over family members must return a complete result.
    """
