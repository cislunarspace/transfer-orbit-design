...
class TestToolRegistry:
    def test_completeness(self):
        """TOOL_REGISTRY 与 e2m2e 工具清单（tool_inventory 跨三类扫描）对齐。"""
        from e2m2e.api import Facade, tool_inventory

        facade_names = {info.name for info in tool_inventory(Facade())}
    ...
    def test_stability_absent_from_inventory(self):
        """e2m2e 5.9.3 清单不再暴露 orbit_stability（上游移除），注册表随之消失。"""
        assert "orbit_stability" not in TOOL_REGISTRY
    ...
    def test_facade_method_matches_tool_key(self):
        """facade_method 是 e2m2e 接口类方法名（== 工具 key，与 tool_inventory 对齐）。"""
        for name, spec in TOOL_REGISTRY.items():
...
class TestDesignOrbitCatalogIngest:
    ...
    def test_export_package_contains_records_and_manifest(
        ...
        assert manifest == {
            # 5.9.3 记录 schema v2（一轨一记录，ADR 0045），导出清单随之
            "schema_version": 2,
