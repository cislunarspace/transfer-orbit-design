...
from OOP.CR3BP_Dynamics import CR3BP_Dynamics
from OOP.CR3BP_System import CR3BP_System
from OOP.Continuation import Continuation
from OOP.DifferentialCorrection import DifferentialCorrection
from OOP.Orbit import Orbit
from OOP.StabilityAnalysis import StabilityAnalysis
from OOP.Visualization import Visualization, ProjectionPlane
...
# 如果有至少一个轨道修正成功，进行延拓
if len(corrected_orbits) > 0:
    # 6. 延拓生成轨道族
    print(f"\n{'=' * 60}")
    print("开始延拓生成轨道族...")
    print(f"{'=' * 60}")

    continuator = Continuation(corrector, param="y_dot0", step=0.01)
    family = continuator.pseudo_arclength_continuation(
        seed_orbit=corrected_orbits[0], steps=50  # 使用第一个修正成功的轨道作为种子
    )

    print(f"延拓完成，生成 {len(family)} 个轨道")

    # 7. 分析稳定性
    print("\n分析轨道稳定性...")
    stability_indices = []
    for i, orbit in enumerate(family):
        analyzer = StabilityAnalysis(orbit)
        stability_index = analyzer.compute_stability_index()
        stability_indices.append(stability_index)
        print(f"轨道 {i + 1}: 稳定性指数 = {stability_index:.6f}")
    ...
    # 绘制轨道族（每5个轨道绘制一个，避免图形过于拥挤）
    for i, orbit in enumerate(family[::5]):  # 每隔5个轨道绘制一个
        color_idx = i % len(viz.color_cycle)
        viz.plot_2D_projection(
            orbit,
            plane=ProjectionPlane.XY,
            color=viz.color_cycle[color_idx],
            label=f"Family {i * 5 + 1}",
            show_start=False,
        )
    ...
    viz.axes.set_title("延拓生成的轨道族", fontsize=14)
    viz.axes.set_xlabel("X")
    viz.axes.set_ylabel("Y")
    viz.axes.grid(True, alpha=0.3)
    viz.axes.legend(loc="best", fontsize=8)
    ...
    viz.save_figure("orbit_family.png")
    viz.show()

    # 绘制稳定性分析图
    viz.figure = plt.figure(figsize=(10, 6))
    viz.axes = viz.figure.add_subplot(111)

    iterations = range(1, len(stability_indices) + 1)
    viz.axes.plot(iterations, stability_indices, "b-o", linewidth=1.5, markersize=4)
    viz.axes.axhline(y=2.0, color="r", linestyle="--", label="Stability boundary (+2)")
    viz.axes.axhline(y=-2.0, color="r", linestyle="--", label="Stability boundary (-2)")
    viz.axes.axhline(y=0, color="gray", linestyle="-", alpha=0.5)

    viz.axes.set_xlabel("Orbit Index")
    viz.axes.set_ylabel("Stability Index")
    viz.axes.set_title("Orbit Family Stability Analysis")
    viz.axes.grid(True, alpha=0.3)
    viz.axes.legend()

    viz.save_figure("stability_analysis.png")
    viz.show()
