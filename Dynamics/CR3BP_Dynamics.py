...
def CR3BP_Omega3_First_Partials(t,SV):
    ...
    # 定义圆型限制性三体模型等效势能
    Omega3 = 0.5*(x**2+y**2+z**2)+(1.0-mu)/r1+mu/r2+0.5*mu*(1.0-mu)
...
def CR3BP_Omega3_Second_Partials(t,SV):
    ...
    ddOmega3_dxx = 1.0+3.0*(1.0-mu)*(x+mu)**2/r1**5+3.0*mu*(1.0-mu-x)**2/r2**5-(1.0-mu)/r1**3-mu/r2**3
    ddOmega3_dxy = 3.0*(1.0-mu)*(x+mu)*y/r1**5-3.0*mu*(1.0-mu-x)*y/r2**5
    ddOmega3_dxz = 3.0*(1.0-mu)*(x+mu)*z/r1**5-3.0*mu*(1.0-mu-x)*z/r2**5
    
    ddOmega3_dyx = ddOmega3_dxy
    ddOmega3_dyy = 1.0+3.0*(1.0-mu)*y**2/r1**5+3.0*mu*y**2/r2**5-(1.0-mu)/r1**3-mu/r2**3
    ddOmega3_dyz = 3.0*(1.0-mu)*y*z/r1**5+3.0*mu*y*z/r2**5
    
    ddOmega3_dzx = ddOmega3_dxz
    ddOmega3_dzy = ddOmega3_dyz
    ddOmega3_dzz = 3.0*(1.0-mu)*z**2/r1**5+3.0*mu*z**2/r2**5-(1.0-mu)/r1**3-mu/r2**3
...
def CR3BP_Propagation(SV0,tf,N):
    ...
    # 数值积分
    output = integrate.solve_ivp(CR3BP_Dynamics,[CR3BP_t[0],CR3BP_t[-1]],SV0,method='DOP853',t_eval=CR3BP_t,rtol=10e-18, atol=10e-18)
...
def CR3BP_Propagation_42(SV0,tf,N):
    ...
    # 数值积分
    output = integrate.solve_ivp(CR3BP_Dynamics_42,[CR3BP_t[0],CR3BP_t[-1]],State_0,method='DOP853',t_eval=CR3BP_t,rtol=10e-18, atol=10e-18)
    
    # 获取状态向量
    CR3BP_State = output.y
    CR3BP_State = CR3BP_State.reshape(42,(np.size(CR3BP_t)),order='F')
    CR3BP_SV = CR3BP_State[0:6,:]
    CR3BP_STM = np.transpose(CR3BP_State[6:42,:]).reshape((np.size(CR3BP_t)),36,1,order='F').reshape((np.size(CR3BP_t)),6,6, order ='C')
