...
def u_bar_first_partials(state, mu):
    ...
    u_bar[0] = -mu * (state[0] - 1.0 + mu) * r_23 - \
...
def cr3bp_eom(t, stateSTM, mu):
    ...
    A[3:6, 3:6] = np.array([[0., 2.0, 0.], [-2.0, 0., 0.], [0., 0., 0.]])
    ...
    # output
    stateSTMdot = np.concatenate((np.array([vx, vy, vz, ax, ay, az]), STMdot.reshape(36)), axis=0)
