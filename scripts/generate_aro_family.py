...
if not bifurcation_points:
    ...
    for i, orbit in enumerate(family_32.orbits):
        try:
            analysis = e2m2e.algorithms.StabilityAnalysis(orbit=orbit, dynamics=dynamics)
    ...
    if best_orbit_idx is not None:
        print(f"找到最近的点: 索引={best_orbit_idx}, x0={family_32.orbits[best_orbit_idx].states[0][0]:.4f}, "
              f"|λ-1|={min_diff:.2e}")
    ...
    for bp in bifurcation_points:
        idx = bp["orbit_index"]
        if idx not in unique_bps or bp["eigenvalue_diff"] < unique_bps[idx]["eigenvalue_diff"]:
            unique_bps[idx] = bp
    ...
    for bp in bifurcation_points[:5]:  # 只显示前5个
        ...
        x0 = orbit.states[0][0]
        z0 = orbit.states[0][2]
        print(f"  索引: {bp['orbit_index']}, x0={x0:.4f}, z0={z0:.4f}, "
...
if bifurcation_points:
    ...
    if aro_bp:
        print(f"找到 ARO 分岔点: 索引={aro_bp['orbit_index']}, "
...
try:
    ...
    print(f"ARO 种子轨道修正成功: x0={seed_orbit_aro.states[0][0]:.4f}, "
