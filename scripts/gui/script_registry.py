...
SCRIPTS: dict[str, list[ScriptEntry]] = {
    "DRO": [
        ScriptEntry(
            ...
            cli_params=[
                CliParam("--x0", "初始 x 坐标", "float", "1.1202", "初始 x 坐标（无量纲）", unit_group="distance"),
                CliParam("--vy0", "初始 vy 速度", "float", "-0.4618", "初始 y 方向速度（无量纲）", unit_group="velocity"),
                CliParam("--period", "目标周期", "float", "2.095", "目标周期（无量纲）", unit_group="time"),
            ],
        ),
        ScriptEntry(
            ...
            output_dir="output/dro",
            cli_params=[
                CliParam("--x0", "初始 x 坐标", "float", "0.79188556619742", "种子轨道初始 x 坐标（无量纲）", unit_group="distance"),
                CliParam("--vy0", "初始 vy 速度", "float", "0.53682", "种子轨道初始 vy 速度（无量纲）", unit_group="velocity"),
                CliParam("--period", "初始周期", "float", "3.472526005624708", "初始周期猜测（无量纲）", unit_group="time"),
                CliParam("--param-min", "延拓下限", "float", "0.141886", "延拓参数范围下限（x0 最小值）", unit_group="distance"),
                CliParam("--param-max", "延拓上限", "float", "0.9", "延拓参数范围上限（x0 最大值）", unit_group="distance"),
    ...
    "RO": [
        ScriptEntry(
            ...
            cli_params=[
                CliParam("--x0", "初始 x 坐标", "float", "-0.8805", "初始 x 坐标（无量纲）", unit_group="distance"),
                CliParam("--vy0", "初始 vy 速度", "float", "0.3921", "初始 y 方向速度（无量纲）", unit_group="velocity"),
                CliParam("--period", "目标周期", "float", str(round(27.32 / 0.3482, 6)), "目标周期（无量纲）", unit_group="time"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                CliParam("--x0", "初始 x 坐标", "float", "-0.8805", "初始 x 坐标（无量纲）", unit_group="distance"),
                CliParam("--vy0", "初始 vy 速度", "float", "0.3921", "初始 y 方向速度（无量纲）", unit_group="velocity"),
                CliParam("--period", "轨道周期", "float", str(round(27.32 / 0.3482, 6)), "轨道周期（无量纲）", unit_group="time"),
                CliParam("--param-min", "延拓下限", "float", "-0.8905", "延拓参数范围下限", unit_group="distance"),
                CliParam("--param-max", "延拓上限", "float", "-0.8305", "延拓参数范围上限", unit_group="distance"),
        ...
        ScriptEntry(
            ...
            output_dir="output/ro",
            cli_params=[
                CliParam("--x0", "初始 x 坐标", "float", "-1.1453", "初始 x 坐标（无量纲）", unit_group="distance"),
                CliParam("--vy0", "初始 vy 速度", "float", "0.4633", "初始 y 方向速度（无量纲）", unit_group="velocity"),
                CliParam("--period", "轨道周期", "float", str(round(54.64 / 0.3482, 6)), "轨道周期（无量纲）", unit_group="time"),
                CliParam("--param-min", "延拓下限", "float", "-1.2", "延拓参数范围下限", unit_group="distance"),
                CliParam("--param-max", "延拓上限", "float", "-0.8", "延拓参数范围上限", unit_group="distance"),
                CliParam("--step-size", "延拓步长", "float", "0.005", "延拓步长"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--target-x0", "目标 x0", "float", "-1.0878", "目标 x0 分岔点", unit_group="distance"),
                CliParam("--z0", "固定 z0", "float", "0.1999", "固定 z0 坐标（无量纲）", unit_group="distance"),
                CliParam("--vy0", "初始 vy 速度", "float", "0.4", "初始 y 方向速度猜测（无量纲）", unit_group="velocity"),
                CliParam("--period", "初始周期", "float", str(round(60.0 / 0.3482, 6)), "初始周期猜测（无量纲）", unit_group="time"),
                CliParam("--x-min", "x 下限", "float", "-1.2", "延拓 x0 范围下限", unit_group="distance"),
                CliParam("--x-max", "x 上限", "float", "-0.9", "延拓 x0 范围上限", unit_group="distance"),
                CliParam("--step-size", "延拓步长", "float", "0.005", "延拓步长"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                CliParam("--ro-file", "RO 文件", "str", help="3:2 RO 轨道 JSON 文件路径", file_category="ro"),
                CliParam("--target-x0", "目标 x0", "float", "-1.1318", "目标 x0 分岔点", unit_group="distance"),
                CliParam("--z-max", "最大 z 幅值", "float", "0.5", "最大 z 幅值", unit_group="distance"),
    ...
    "Halo": [
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--amplitude-z", "Z 振幅", "float", "0.23", "Z 方向振幅（无量纲）", unit_group="distance"),
                CliParam("--halo-class", "Halo 类型", "int", "0", "0=北 Halo, 1=南 Halo"),
                CliParam("--period", "目标周期", "float", "1.839732", "目标周期（无量纲）", unit_group="time"),
                CliParam("--x0", "初始 x 坐标", "float", "0.9305269194214338", "初始 x 坐标（无量纲）", unit_group="distance"),
    ...
    "Transfer": [
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--max-transfer-time", "最大转移时间", "float", str(round(100.0 / 0.3482, 6)), "最大转移时间（无量纲）", unit_group="time"),
                CliParam("--intersection-threshold", "相交阈值", "float", "0.001", "相交判定距离阈值", unit_group="distance"),
                CliParam("--min-distance", "最小距离阈值", "float", str(round(100.0 / 384400, 6)), "候选解最小距离阈值", unit_group="distance"),
                CliParam("--earth-radius", "地球半径", "float", str(round(200.0 / 384400, 6)), "地球碰撞检测半径", unit_group="distance"),
                CliParam("--moon-radius", "月球半径", "float", str(round(100.0 / 384400, 6)), "月球碰撞检测半径", unit_group="distance"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--alpha-max", "alpha 上界", "float", "2.5", "alpha 搜索上界"),
                CliParam("--max-transfer-time", "最大转移时间", "float", str(round(100.0 / 0.3482, 6)), "最大转移时间（无量纲）", unit_group="time"),
                CliParam("--geo-threshold", "GEO 相交阈值", "float", str(round(100.0 / 384400, 6)), "GEO 相交距离阈值", unit_group="distance"),
                CliParam("--earth-radius", "地球半径", "float", str(round(200.0 / 384400, 6)), "地球碰撞检测半径", unit_group="distance"),
                CliParam("--moon-radius", "月球半径", "float", str(round(100.0 / 384400, 6)), "月球碰撞检测半径", unit_group="distance"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--max-transfer-time", "最大转移时间", "float", str(round(10.0 / 0.3482, 6)), "最大转移时间（无量纲）", unit_group="time"),
                CliParam("--intersection-threshold", "相交阈值", "float", str(round(100.0 / 384400, 6)), "相交判定距离阈值", unit_group="distance"),
                CliParam("--min-distance", "最小距离阈值", "float", str(round(100.0 / 384400, 6)), "候选解最小距离阈值", unit_group="distance"),
                CliParam("--earth-radius", "地球半径", "float", str(round(200.0 / 384400, 6)), "地球碰撞检测半径", unit_group="distance"),
                CliParam("--moon-radius", "月球半径", "float", str(round(100.0 / 384400, 6)), "月球碰撞检测半径", unit_group="distance"),
        ...
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--max-transfer-time", "最大转移时间", "float", "80.0", "最大转移时间（无量纲）", unit_group="time"),
                CliParam("--intersection-threshold", "相交阈值", "float", "0.001", "相交判定距离阈值", unit_group="distance"),
                CliParam("--min-distance", "最小距离阈值", "float", str(round(500.0 / 384400, 6)), "候选解最小距离阈值", unit_group="distance"),
                CliParam("--earth-radius", "地球半径", "float", str(round(200.0 / 384400, 6)), "地球碰撞检测半径", unit_group="distance"),
                CliParam("--moon-radius", "月球半径", "float", str(round(100.0 / 384400, 6)), "月球碰撞检测半径", unit_group="distance"),
        ...
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--alpha-min", "alpha 下界", "float", "0.5", "alpha 搜索下界"),
                CliParam("--alpha-max", "alpha 上界", "float", "2.5", "alpha 搜索上界"),
                CliParam("--t-min", "转移时间下界", "float", "0.5", "转移时间下界（无量纲）", unit_group="time"),
                CliParam("--t-max", "转移时间上界", "float", "30.0", "转移时间上界（无量纲）", unit_group="time"),
                CliParam("--nlp-maxiter", "NLP 最大迭代", "int", "100", "NLP 最大迭代次数"),
                CliParam("--nlp-ftol", "NLP 函数容差", "float", "1e-8", "NLP 函数容差"),
                CliParam("--top-k", "前 K 个可行解", "int", help="取前 K 个可行解优化"),
                CliParam("--max-cases", "最大案例数", "int", help="最大优化案例数"),
        ...
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--alpha-min", "alpha 下界", "float", "1.0", "alpha 搜索下界"),
                CliParam("--alpha-max", "alpha 上界", "float", "1.5", "alpha 搜索上界"),
                CliParam("--t-min", "转移时间下界", "float", "5.0", "转移时间下界（无量纲）", unit_group="time"),
                CliParam("--t-max", "转移时间上界", "float", "60.0", "转移时间上界（无量纲）", unit_group="time"),
                CliParam("--t-ins-min", "插入时间下界", "float", "0.0", "DRO 插入时间下界", unit_group="time"),
                CliParam("--t-ins-max", "插入时间上界", "float", "10.0", "DRO 插入时间上界", unit_group="time"),
        ...
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--alpha-max", "alpha 上界", "float", "2.0", "alpha 搜索上界"),
                CliParam("--t-min", "转移时间下界", "float", "5.0", "转移时间下界（无量纲）", unit_group="time"),
                CliParam("--t-max", "转移时间上界", "float", "80.0", "转移时间上界（无量纲）", unit_group="time"),
                CliParam("--t-ins-min", "插入时间下界", "float", "0.0", "DRO 插入时间下界", unit_group="time"),
                CliParam("--t-ins-max", "插入时间上界", "float", "10.0", "DRO 插入时间上界", unit_group="time"),
                CliParam("--velocity-angle-tol", "速度平行性容差", "float", help="速度平行性容差（度）", unit_group="angle"),
                CliParam("--nlp-maxiter", "NLP 最大迭代", "int", "100", "NLP 最大迭代次数"),
