...
SCRIPTS: dict[str, list[ScriptEntry]] = {
    "DRO": [
        ScriptEntry(
            ...
            cli_params=[
                CliParam("--x0", "初始 x 坐标", "float", "1.1202", "初始 x 坐标（无量纲）"),
                CliParam("--vy0", "初始 vy 速度", "float", "-0.4618", "初始 y 方向速度（无量纲）"),
                CliParam("--period", "目标周期", "float", "2.095", "目标周期（无量纲）"),
            ],
        ),
        ScriptEntry(
    ...
    "RO": [
        ScriptEntry(
            ...
            cli_params=[
                CliParam("--x0", "初始 x 坐标", "float", "-0.8805", "初始 x 坐标（无量纲）"),
                CliParam("--vy0", "初始 vy 速度", "float", "0.3921", "初始 y 方向速度（无量纲）"),
                CliParam("--period", "目标周期", "float", str(round(27.32 / 0.3482, 6)), "目标周期（无量纲）"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                CliParam("--x0", "初始 x 坐标", "float", "-0.8805", "初始 x 坐标（无量纲）"),
                CliParam("--vy0", "初始 vy 速度", "float", "0.3921", "初始 y 方向速度（无量纲）"),
                CliParam("--period", "轨道周期", "float", str(round(27.32 / 0.3482, 6)), "轨道周期（无量纲）"),
                CliParam("--param-min", "延拓下限", "float", "-0.8905", "延拓参数范围下限"),
                CliParam("--param-max", "延拓上限", "float", "-0.8305", "延拓参数范围上限"),
        ...
        ScriptEntry(
            ...
            output_dir="output/ro",
            cli_params=[
                CliParam("--x0", "初始 x 坐标", "float", "-1.1453", "初始 x 坐标（无量纲）"),
                CliParam("--vy0", "初始 vy 速度", "float", "0.4633", "初始 y 方向速度（无量纲）"),
                CliParam("--period", "轨道周期", "float", str(round(54.64 / 0.3482, 6)), "轨道周期（无量纲）"),
                CliParam("--param-min", "延拓下限", "float", "-1.2", "延拓参数范围下限"),
                CliParam("--param-max", "延拓上限", "float", "-0.8", "延拓参数范围上限"),
                CliParam("--step-size", "延拓步长", "float", "0.005", "延拓步长"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--period", "初始周期", "float", str(round(60.0 / 0.3482, 6)), "初始周期猜测（无量纲）"),
                CliParam("--x-min", "x 下限", "float", "-1.2", "延拓 x0 范围下限"),
                CliParam("--x-max", "x 上限", "float", "-0.9", "延拓 x0 范围上限"),
                CliParam("--step-size", "延拓步长", "float", "0.005", "延拓步长"),
            ],
        ),
        ScriptEntry(
    ...
    "Halo": [
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--amplitude-z", "Z 振幅", "float", "0.23", "Z 方向振幅（无量纲）"),
                CliParam("--halo-class", "Halo 类型", "int", "0", "0=北 Halo, 1=南 Halo"),
                CliParam("--period", "目标周期", "float", "1.839732", "目标周期（无量纲）"),
                CliParam("--x0", "初始 x 坐标", "float", "0.9305269194214338", "初始 x 坐标（无量纲）"),
                CliParam("--vy0", "初始 vy 速度", "float", "0.10431508546142665", "初始 y 方向速度（无量纲）"),
    ...
    "Transfer": [
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--max-transfer-time", "最大转移时间", "float", str(round(100.0 / 0.3482, 6)), "最大转移时间（无量纲）"),
                CliParam("--intersection-threshold", "相交阈值", "float", "0.001", "相交判定距离阈值"),
                CliParam("--min-distance", "最小距离阈值", "float", str(round(100.0 / 384400, 6)), "候选解最小距离阈值"),
                CliParam("--earth-radius", "地球半径", "float", str(round(200.0 / 384400, 6)), "地球碰撞检测半径"),
                CliParam("--moon-radius", "月球半径", "float", str(round(100.0 / 384400, 6)), "月球碰撞检测半径"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--alpha-max", "alpha 上界", "float", "2.5", "alpha 搜索上界"),
                CliParam("--max-transfer-time", "最大转移时间", "float", str(round(100.0 / 0.3482, 6)), "最大转移时间（无量纲）"),
                CliParam("--geo-threshold", "GEO 相交阈值", "float", str(round(100.0 / 384400, 6)), "GEO 相交距离阈值"),
                CliParam("--earth-radius", "地球半径", "float", str(round(200.0 / 384400, 6)), "地球碰撞检测半径"),
                CliParam("--moon-radius", "月球半径", "float", str(round(100.0 / 384400, 6)), "月球碰撞检测半径"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--max-transfer-time", "最大转移时间", "float", str(round(10.0 / 0.3482, 6)), "最大转移时间（无量纲）"),
                CliParam("--intersection-threshold", "相交阈值", "float", str(round(100.0 / 384400, 6)), "相交判定距离阈值"),
                CliParam("--min-distance", "最小距离阈值", "float", str(round(100.0 / 384400, 6)), "候选解最小距离阈值"),
                CliParam("--earth-radius", "地球半径", "float", str(round(200.0 / 384400, 6)), "地球碰撞检测半径"),
                CliParam("--moon-radius", "月球半径", "float", str(round(100.0 / 384400, 6)), "月球碰撞检测半径"),
        ...
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--max-transfer-time", "最大转移时间", "float", "80.0", "最大转移时间（无量纲）"),
                CliParam("--intersection-threshold", "相交阈值", "float", "0.001", "相交判定距离阈值"),
                CliParam("--min-distance", "最小距离阈值", "float", str(round(500.0 / 384400, 6)), "候选解最小距离阈值"),
                CliParam("--earth-radius", "地球半径", "float", str(round(200.0 / 384400, 6)), "地球碰撞检测半径"),
                CliParam("--moon-radius", "月球半径", "float", str(round(100.0 / 384400, 6)), "月球碰撞检测半径"),
        ...
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--velocity-angle-tol", "速度方向容差", "float", "0.05", "速度方向容差（弧度）"),
            ],
        ),
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--t-max", "转移时间上界", "float", "30.0", "转移时间上界（无量纲）"),
                CliParam("--nlp-maxiter", "NLP 最大迭代", "int", "100", "NLP 最大迭代次数"),
                CliParam("--nlp-ftol", "NLP 函数容差", "float", "1e-8", "NLP 函数容差"),
                CliParam("--top-k", "前 K 个可行解", "int", help="取前 K 个可行解优化"),
                CliParam("--max-cases", "最大案例数", "int", help="最大优化案例数"),
        ...
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--t-max", "转移时间上界", "float", "60.0", "转移时间上界（无量纲）"),
                CliParam("--t-ins-min", "插入时间下界", "float", "0.0", "DRO 插入时间下界"),
                CliParam("--t-ins-max", "插入时间上界", "float", "10.0", "DRO 插入时间上界"),
                CliParam("--velocity-angle-tol", "速度平行性容差", "float", help="速度平行性容差（度）"),
                CliParam("--nlp-maxiter", "NLP 最大迭代", "int", "100", "NLP 最大迭代次数"),
                CliParam("--nlp-ftol", "NLP 函数容差", "float", "1e-8", "NLP 函数容差"),
                CliParam("--top-k", "前 K 个可行解", "int", help="取前 K 个可行解优化"),
                CliParam("--max-cases", "最大案例数", "int", help="最大优化案例数"),
        ...
        ScriptEntry(
            ...
            cli_params=[
                ...
                CliParam("--alpha-max", "alpha 上界", "float", "2.0", "alpha 搜索上界"),
                CliParam("--t-min", "转移时间下界", "float", "5.0", "转移时间下界（无量纲）"),
                CliParam("--t-max", "转移时间上界", "float", "80.0", "转移时间上界（无量纲）"),
                CliParam("--t-ins-min", "插入时间下界", "float", "0.0", "DRO 插入时间下界"),
                CliParam("--t-ins-max", "插入时间上界", "float", "10.0", "DRO 插入时间上界"),
                CliParam("--velocity-angle-tol", "速度平行性容差", "float", help="速度平行性容差（度）"),
                CliParam("--nlp-maxiter", "NLP 最大迭代", "int", "100", "NLP 最大迭代次数"),
