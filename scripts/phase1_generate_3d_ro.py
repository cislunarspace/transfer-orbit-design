...
def gradual_z_search(
    dynamics, planar_state, t_half, z_target, n_steps=40, verbose=True
):
        ...
        if result["success"]:
            ...
            if valid and abs(result["state"][2]) > 1e-6:
                ...
                if verbose and (i + 1) % 5 == 0:
                    s = result["state"]
                    print(
                        f"    z0={z0:.4f}: x0={s[0]:.6f}, z0_act={s[2]:.6f}, T={result['period']:.6f}"
                    )
    ...
    for z0_start in [0.001, 0.005, 0.01, 0.02, 0.05]:
        ...
        if result["success"]:
            ...
            if abs(z0_actual) > 1e-6:
                valid, msg = validate_3d_orbit(
                    result["state"], result["period"], dynamics
                )
                if valid:
                    if verbose:
                        print(
                            f"    起始3D轨道: z0={z0_actual:.6f}, T={result['period']:.6f}"
                        )
                    ...
                    cont.direction = (
                        ContinuationDirection.FORWARD
                        if step > 0
                        else ContinuationDirection.BACKWARD
                    )
...
def main():
    ...
    for label, p, q in [("3:2", 3, 2), ("3:1", 3, 1)]:
        ...
        print(
            f"  平面RO: x0={planar_state[0]:.8f}, vy0={planar_state[4]:.8f}, T={planar_T:.8f}"
        )
        found_for_resonance = []
        ...
        if result["success"]:
            valid, msg = validate_3d_orbit(result["state"], result["period"], dynamics)
            if valid:
                otype, corr, zmax = classify_3d_orbit(
                    dynamics, result["state"], result["period"]
                )
                ...
                found_for_resonance.append(
                    {
                        ...
                        "z_max": zmax,
                        "orbit_type": otype,
                        "base_resonance": label,
                        "jacobi": C,
                        "label": f"{label} {otype}",
                    }
                )
                print(
                    f"  OK: {otype} z_max={zmax:.4f} T={result['period']:.6f} C={C:.6f}"
                )
            else:
                print(f"  Invalid: {msg}")
        else:
        ...
        res_grad = gradual_z_search(
            dynamics, planar_state, t_half, AZ_TARGET, n_steps=40, verbose=True
        )
        if res_grad:
            ...
            best = res_grad[idx]
            is_dup = any(
                np.linalg.norm(e["state"] - best["state"]) < 0.01
                for e in found_for_resonance
            )
            if not is_dup:
                otype, corr, zmax = classify_3d_orbit(
                    dynamics, best["state"], best["period"]
                )
        ...
        for dvy in [-0.2, -0.1, 0.1, 0.2, -0.3, 0.3, -0.5, 0.5]:
            ...
            if r2["success"]:
                ...
                if v:
                    is_dup = any(
                        np.linalg.norm(e["state"] - r2["state"]) < 0.01
                        for e in found_for_resonance
                    )
                    if not is_dup:
                        otype, corr, zmax = classify_3d_orbit(
                            dynamics, r2["state"], r2["period"]
                        )
                        ...
                        found_for_resonance.append(
                            {
                                ...
                                "base_resonance": label,
                                "jacobi": C,
                                "label": f"{label} {otype} (dvy={dvy:+.1f})",
                            }
                        )
                        print(
                            f"  OK (dvy={dvy:+.1f}): {otype} z={r2['state'][2]:.4f} T={r2['period']:.6f}"
                        )
    ...
    for info in all_3d:
        ...
        if r["success"]:
            ...
            if v:
                ...
                is_dup = any(
                    np.linalg.norm(e["state"] - r["state"]) < 0.01 for e in final
                )
                if not is_dup:
                    final.append(
                        {
                    ...
                    print(
                        f"  {info['label']}: x0={r['state'][0]:.6f} z0={r['state'][2]:.6f} "
    ...
    for o in final:
        print(
            f"  {o['label']}: T={o['period']:.4f}, z_max={o['z_max']:.4f}, C={o['jacobi']:.4f}"
        )
    if not final:
        print(
            "  No valid 3D RO found. This is expected if no bifurcation exists at these resonances."
        )
    print("Done!")
