...
plt.tight_layout()
plt.savefig(
    project_root / "output" / "ro" / f"{family_name}_2d_view.png",
    dpi=300,
    bbox_inches="tight",
)
plt.show()
...
# 全局3D视图参数
global_center_x = -0.85  # 3:2 ARO 中心位置
global_center_y = 0.0
global_center_z = 0.2  # ARO 有更大的 z 幅值
global_radius = 0.5

# 绘制种子轨道
label_3d = f"Seed 3:2 ARO (C={seed_jacobi:.4f})"
...
# 设置视角
ax_global_3d.view_init(elev=20, azim=-90)

plt.tight_layout()
plt.savefig(
    project_root / "output" / "ro" / f"{family_name}_3d_view.png",
    dpi=300,
    bbox_inches="tight",
)
plt.show()
...
plt.tight_layout()
plt.savefig(
    project_root / "output" / "ro" / f"{family_name}_period_stability.png",
    dpi=300,
    bbox_inches="tight",
)
plt.show()
