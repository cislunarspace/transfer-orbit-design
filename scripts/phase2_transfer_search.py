...
def _make_earth_collision():
    def event(t, state):
        return (
            np.sqrt((state[0] + MU) ** 2 + state[1] ** 2 + state[2] ** 2)
            - R_EARTH_ND * 1.5
        )
...
def _make_moon_collision():
    def event(t, state):
        return (
            np.sqrt((state[0] - 1 + MU) ** 2 + state[1] ** 2 + state[2] ** 2)
            - R_MOON_ND * 1.5
        )
...
def search_transfers(
    ...
    if verbose:
        ...
        print(
            f"  DRO  x0={dro_state[0]:.6f}  T={dro_period:.6f} "
            f"({dro_period * TU:.2f} d)"
        )
        print(
            f"  RO   x0={ro_state[0]:.6f}  T={ro_period:.6f} ({ro_period * TU:.2f} d)"
        )
    ...
    for i_dep in range(n_dep):
        ...
        if verbose and (i_dep + 1) % max(1, n_dep // 6) == 0:
            ...
            print(
                f"  {pct:5.1f}%  {len(candidates):5d} cands  "
...
def optimize_candidate(cand, ro_sol, ro_period, verbose=False):
    ...
    for T in T_grid:
        ...
        for tins in tins_grid:
            n_eval[0] += 1
            result = _refine_root(
                r_dep, v_dro, e_t, e_n, alpha0, dvm0, T, tins, ro_sol, ro_period
            )
...
def plot_solution_plane(results, pair_name, outdir):
    ...
    for r, td, dv in zip(results, Tdays, dvtot):
        ...
        ax.scatter(
            td, dv, c=c, s=30, zorder=3, label=lbl if lbl not in labels_placed else ""
        )
    ...
    fig.tight_layout()
    fig.savefig(
        str(Path(outdir) / f"solution_plane_{_safe_filename(pair_name)}.png"), dpi=150
    )
    plt.close(fig)


def plot_sample_transfers(
    classified, dro_state, dro_period, ro_state, ro_period, pair_name, outdir
):
    ...
    fig.tight_layout()
    fig.savefig(
        str(Path(outdir) / f"transfers_{_safe_filename(pair_name)}.png"), dpi=150
    )
    plt.close(fig)
...
def load_phase1_data():
    ...
    return {
        "2:1 DRO": {"state": np.array(dro_21["state"]), "period": dro_21["period"]},
        "3:1 DRO": {"state": np.array(dro_31["state"]), "period": dro_31["period"]},
        "3:2 RO": {"state": np.array(ro_32["state"]), "period": ro_32["period"]},
        "3:1 RO": {"state": np.array(ro_31["state"]), "period": ro_31["period"]},
    }
...
def process_one_pair(
    ...
    for c in sorted(classified, key=lambda c: c["opt"]["total_dv"])[:5]:
        print(
            f"    {c['type']:12s}  ΔV={c['opt']['total_dv'] * VU:7.1f} m/s  "
...
def main():
    ...
    for pair_name, results in all_results.items():
        ...
        if (
            ...
            and "opt" in results[0]
        ):
            best = min(results, key=lambda c: c["opt"]["total_dv"])
            tprint(f"  {pair_name}")
            tprint(f"    {len(results)} solutions found")
            tprint(
                f"    Best: dV={best['opt']['total_dv'] * VU:.1f} m/s  "
            ...
            tprint(
                f"    Best: dV={best['total_dv'] * VU:.1f} m/s  "
