...
def find_ro_seed(dynamics, p, q, verbose=True):
    ...
    if orbit is not None:
        ...
        if x0_result < 1 - MU - 0.1:
            if verbose:
                print(f"  ✓ 方法1成功: x0={x0_result:.8f}, vy0={result['state'][4]:.8f}")
            return orbit, result
        elif verbose:
    ...
    for x0_try in np.arange(0.1, 0.85, 0.02):
        ...
        for vy_factor in [0.3, 0.5, 0.7, 0.9, 1.0, 1.1]:
            ...
            if orbit is not None:
                ...
                if 0 < x0_r < 1 - MU - 0.1:
                    if verbose:
                        print(f"  ✓ 方法4成功 (x0_g={x0_try:.2f}, vy_f={vy_factor:.1f}): "
...
def plot_ro_family(family_data, label, target_T, save_dir, system):
    ...
    if 'target' in family_data:
        ...
        ax.plot(res.y[0], res.y[1], 'r-', linewidth=2.5,
                label=f'{label} target (T={T:.4f})')
    ...
    fig.savefig(save_dir / f'{label.replace(":", "_")}_family_orbits.png',
    ...
    ax.axhline(y=target_T, color='r', linestyle='--', alpha=0.7, label=f'T_target={target_T:.4f}')
    ax.set_xlabel('$x_0$')
    ...
    fig.tight_layout()
    fig.savefig(save_dir / f'{label.replace(":", "_")}_family_characteristics.png',
                dpi=200, bbox_inches='tight')
    plt.close(fig)
...
def process_one_resonance(system, dynamics, p, q, n_out=30, n_in=20):
    ...
    # 2. 生成族 (延拓)
    family = generate_ro_family(dynamics, seed_result,
                                n_outward=n_out, n_inward=n_in, verbose=True)
    ...
    print(f"  最佳初始猜测: 索引={idx_best}, T={best_T:.6f}, ΔT={abs(best_T - T_target):.2e}")
...
def main():
    ...
    for key, fam in results.items():
        ...
        print(f"  x0范围: [{fam['states'][:, 0].min():.4f}, {fam['states'][:, 0].max():.4f}]")
        print(f"  周期范围: [{fam['periods'].min():.4f}, {fam['periods'].max():.4f}]")
        if 'target' in fam:
            t = fam['target']
            print(f"  目标轨道: x0={t['state'][0]:.8f}, vy0={t['state'][4]:.8f}, T={t['period']:.8f}")
