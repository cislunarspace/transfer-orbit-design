...
# 添加项目根目录到路径
sys.path.insert(0, str(Path(__file__).parent.parent.parent / 'e2m2e'))
...
# CR3BP参数 (Earth-Moon system)
MU = 1.21506683e-2  # Gravitational parameter ratio
...
def main():
    ...
    # 保存可行解详情
    if feasible:
        feasible_file = output_dir / f'feasible_v2_{dro_name}_{ro_name}_{timestamp}.json'
        ...
        for i, r in enumerate(feasible_sorted):
            print(f"  {i+1}. α={r.alpha:.4f}, β={r.beta:.4f}, "
