...
def parse_args() -> argparse.Namespace:
    ...
    Returns:
        解析后的命令行参数命名空间。
    """
    parser = argparse.ArgumentParser(description="DRO→GEO 转移轨道网格搜索", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument(
...
SCRIPT_ENTRY = ScriptEntry(
    ...
    cli_params=[
        ...
        CliParam('--n-departure', '出发点数', 'int', '200', help='出发时间网格数。'),
        CliParam('--n-alpha', 'alpha 密度', 'int', '100', help='alpha 网格密度。'),
        CliParam('--alpha-min', 'alpha 下界', 'float', '0.5', help='alpha 搜索下界。'),
        CliParam('--alpha-max', 'alpha 上界', 'float', '2.5', help='alpha 搜索上界。'),
        CliParam('--max-transfer-time', '最大转移时间', 'float', '22.998482', help='最大转移时间（无量纲）。', unit_group='time', default_unit='days'),
        CliParam('--intersection-threshold', 'GEO 相交阈值', 'float', '0.00026', help='GEO 相交距离阈值，单位 km。', unit_group='distance', default_unit='km'),
        CliParam('--min-distance', '最小距离阈值', 'float', '0.00026', help='候选解最小距离阈值，单位 km。', unit_group='distance', default_unit='km'),
        CliParam('--geo-n-points', 'GEO 采样点数', 'int', '1000', help='GEO 轨道采样点数。'),
        CliParam('--earth-radius', '地球半径', 'float', '0.00052', help='地球碰撞检测半径，单位 km。', unit_group='distance', default_unit='km'),
        CliParam('--moon-radius', '月球半径', 'float', '0.00026', help='月球碰撞检测半径，单位 km。', unit_group='distance', default_unit='km'),
    ],
)
