...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        sys.argv += [
            ...
            "--intersection-threshold", "0.001",          # 相交判定距离阈值
            "--min-distance", "0.0002601422978369168",    # 候选解最小距离阈值（100.0/DU）
...
SCRIPT_ENTRY = ScriptEntry(
    ...
    cli_params=[
        ...
        CliParam('--max-transfer-time', '最大转移时间', 'float', '287.191269', help='最大转移时间（无量纲）。', unit_group='time', default_unit='days'),
        CliParam('--intersection-threshold', '相交阈值', 'float', '0.001', help='相交判定距离阈值，单位 km。', unit_group='distance', default_unit='km'),
