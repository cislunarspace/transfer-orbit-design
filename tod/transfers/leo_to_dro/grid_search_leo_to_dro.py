...
SCRIPT_ENTRY = ScriptEntry(
    ...
    cli_params=[
        ...
        CliParam('--max-transfer-time', '最大转移时间', 'float', '80.0', help='最大转移时间（无量纲）。', unit_group='time', default_unit='days'),
        CliParam('--intersection-threshold', '相交阈值', 'float', '0.001', help='相交判定距离阈值，单位 km。', unit_group='distance', default_unit='km'),
        CliParam('--min-distance', '最小距离阈值', 'float', '0.001301', help='候选解最小距离阈值，单位 km。', unit_group='distance', default_unit='km'),
        ...
        CliParam('--leo-n-points', 'LEO 采样点数', 'int', '500', help='LEO 轨道采样点数。'),
    ],
)
