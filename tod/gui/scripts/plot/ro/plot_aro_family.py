...
SCRIPT_ENTRY = ScriptEntry(
    module='ro',
    name='plot_aro_family',
    description='可视化 plot aro family 相关结果，帮助检查轨道几何、稳定性、搜索候选或优化质量。脚本读取 GUI 选择的 JSON 文件，并根据勾选项绘制 2D/3D、散点、统计或交互浏览视图。未填写保存路径时弹出 Matplotlib 窗口；填写保存路径时生成图片文件。',
    script_path='tod/plot/ro/plot_aro_family.py',
    ...
    cli_params=[
        ...
        CliParam('--plot-global-2d', '全局 2D 视图（XY 平面）', 'bool', '', help='绘制 ARO 轨道族在 XY 平面的 2D 视图，勾选后启用。'),
        CliParam('--plot-global-3d', '全局 3D 视图', 'bool', '', help='绘制 ARO 轨道族的 3D 示意图，勾选后启用。'),
