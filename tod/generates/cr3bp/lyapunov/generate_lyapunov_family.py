...
本模块在地月 CR3BP 中延拓生成轨道族。通过改变 x 方向振幅，系统生成一系列围绕平动点的平面周期轨道族。
...
def parse_args(argv=None):
    ...
    """
    parser = argparse.ArgumentParser(description="在地月 CR3BP 中生成 Lyapunov 轨道族。通过改变 x 方向振幅，系统生成一系列围绕平动点的平面周期轨道族。", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    ...
    parser.add_argument("--amplitude-x-min", type=float, default=0.01, help="延拓 x 方向振幅下限（无量纲），默认 0.01。")
    parser.add_argument("--amplitude-x-max", type=float, default=0.5, help="延拓 x 方向振幅上限（无量纲），默认 0.5。")
