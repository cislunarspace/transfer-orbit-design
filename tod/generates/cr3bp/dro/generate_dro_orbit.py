...
from tod.generates.cr3bp.importer import OrbitRecord, import_cr3bp_xlsx_catalog, load_cr3bp_catalog
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--jacobi-tolerance", type=float, default=1e-4, help="Jacobi 最近邻匹配容差")
...
def _catalog_seed_metadata(seed: CatalogSeed) -> dict[str, object]:
...
def _ensure_catalog_available(args: argparse.Namespace) -> None:
    ...
    import_cr3bp_xlsx_catalog(args.raw_data_dir, args.catalog_dir, overwrite=True)
...
def _select_catalog_seed(args: argparse.Namespace) -> CatalogSeed:
    ...
    if args.seed_id is not None:
        ...
        raise SystemExit(f"未找到 DRO catalog seed_id={args.seed_id!r}")
...
def _propagate_catalog_seed(initial_state: list[float], period: float, dynamics) -> Orbit:
    times = np.linspace(0.0, period, 1000)
...
def _save_orbit(orbit: Orbit, metadata: dict[str, object]) -> Path:
    orbit.metadata.update(metadata)
    ts = int(time.time())
    output_file = OUTPUT_DIR / f"dro_{ts}.json"
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    if args.seed_id is not None or args.jacobi is not None:
        ...
        orbit_result = _propagate_catalog_seed(seed.initial_state, seed.period, dynamics)
        output_file = _save_orbit(orbit_result, _catalog_seed_metadata(seed))
