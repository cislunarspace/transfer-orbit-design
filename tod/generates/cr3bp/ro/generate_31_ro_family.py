...
运行示例:
    .. code-block:: bash
       ...
       uv run python -m tod.generates.cr3bp.ro.generate_31_ro_family --help
"""
...
def parse_args():
    ...
    Returns:
        解析后的命令行参数命名空间。
    """
    parser = argparse.ArgumentParser(description="生成 3:1 共振轨道族（差分修正 + 自然延拓）", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def main():
    ...
    family_result.save_to_file(
        filename=str(
            OUTPUT_DIR
            / "ro"
            / f"ro_31_family_{param_min}-{param_max}-{step_size}_{ts}.json"
        )
    )
    logger.info("已保存至：ro_31_family_%s-%s-%s_%d.json", param_min, param_max, step_size, ts)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 1:
        sys.argv += [
            ...
            "--period", "6.283185307179586",              # 轨道周期（无量纲，27.32/TU）
            "--param-min", "-0.8905",                     # 延拓参数范围下限
            "--param-max", "-0.8305",                     # 延拓参数范围上限
            "--step-size", "0.001",                       # 延拓步长
        ]
