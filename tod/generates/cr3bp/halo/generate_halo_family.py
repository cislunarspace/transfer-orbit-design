...
SCRIPT_ENTRY = ScriptEntry(
    ...
    cli_params=[
        ...
        CliParam('--amplitude-z', 'Z 振幅', 'float', '0.001', help='Z 方向振幅（种子轨道），单位 DU。', unit_group='distance', default_unit='DU', advanced=True, hidden_when='--seed-file'),
        CliParam('--method', '延拓方法', 'str', '伪弧长延拓', help='延拓方法选择', choices=('伪弧长延拓',), choice_values={'伪弧长延拓': 'pseudo_arclength'}),
        CliParam('--step-size-pal', '伪弧长延拓步长', 'float', '0.0045', help='伪弧长延拓步长 |Δs|，单位 DU。', unit_group='distance', default_unit='DU', hidden_when='--method==natural'),
        CliParam('--n-orbits', '轨道数量', 'int', '20', help='延拓轨道数量。', hidden_when='--method==natural'),
        CliParam('--step-size', '延拓步长', 'float', '0.002', help='自然延拓步长，单位 DU。', unit_group='distance', default_unit='DU', advanced=True, hidden_when='--method==pseudo_arclength'),
