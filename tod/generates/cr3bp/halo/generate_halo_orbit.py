...
SCRIPT_ENTRY = ScriptEntry(
    ...
    group_label='生成',
    cli_params=[
        CliParam('--libration-point', '平动点', 'str', 'L1', help='平动点：L1, L2。', choices=('L1', 'L2')),
        CliParam('--amplitude-z', 'Z 振幅', 'float', '0.23', help='Z 方向振幅，单位 DU。', unit_group='distance', default_unit='DU'),
        CliParam('--halo-class', 'Halo 类型', 'str', '北族', help='Halo 轨道族类型。', choices=('北族', '南族'), choice_values={'北族': '0', '南族': '1'}),
        CliParam('--period', '目标周期', 'float', '1.839732', help='目标周期（无量纲）。', unit_group='time', default_unit='days'),
        CliParam('--x0', '初始 x 坐标', 'float', '0.9305269194214338', help='初始 x 坐标，单位 DU。', unit_group='distance', default_unit='DU'),
        CliParam('--vy0', '初始 vy 速度', 'float', '0.10431508546142665', help='初始 y 方向速度。', unit_group='velocity', default_unit='VU'),
        CliParam('--max-iterations', '最大迭代次数', 'int', '150', help='最大迭代次数。'),
        CliParam('--tolerance', '修正容差', 'float', '1e-6', help='修正容差。'),
    ],
)
