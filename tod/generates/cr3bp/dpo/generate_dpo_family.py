...
本模块在地月 CR3BP 中构造 DPO 种子轨道，调用 e2m2e 的微分修正和自然延拓算法
生成 DPO 轨道族。输入为命令行给出的初始状态、周期猜测和延拓配置；
输出为 output/dpo/ 下的 JSON/CSV 文件。
...
class DpoFamilyGenerator(FamilyGenerator):
    ...
    继承 ``FamilyGenerator`` 基类，实现 DPO 特有的种子构造、
    微分修正器配置和自然延拓逻辑。
    """
    ...
    def _get_seed_orbit(self, args: Any) -> Orbit:
        ...
        DPO 特征：平面内运动（y=z=0），关于 x 轴对称（vx=vz=0）。
        """
        x0 = args.x0
        vy0 = args.vy0
    ...
    def _build_json_filename(self, args: Any, ts: int) -> str:
        """DPO 文件名包含延拓参数范围，匹配原始输出格式。"""
        return (
            f"dpo_family_{args.param_min}-{args.param_max}-{args.step_size}_{ts}"
        )

    def _build_csv_filename_parts(self, args: Any, ts: int) -> list[str]:
        """DPO CSV 文件名前缀片段（不含 ts），匹配原始输出格式。
...
SCRIPT_ENTRY = ScriptEntry(
    ...
    group_label='生成',
    cli_params=[
        CliParam('--x0', '初始 x 坐标', 'float', '1.03774', help='种子轨道初始 x 坐标（无量纲）。', unit_group='distance', default_unit='DU'),
        CliParam('--vy0', '初始 vy 速度', 'float', '0.503284', help='种子轨道初始 vy 速度（无量纲）。', unit_group='velocity'),
        CliParam('--period', '初始周期', 'float', '1.2011', help='初始周期猜测（无量纲）。', unit_group='time', default_unit='days'),
        CliParam('--param-min', '延拓下限', 'float', '0.997', help='延拓参数范围下限（x0 最小值），单位 DU。', unit_group='distance', default_unit='DU'),
        CliParam('--param-max', '延拓上限', 'float', '1.046', help='延拓参数范围上限（x0 最大值），单位 DU。', unit_group='distance', default_unit='DU'),
        CliParam('--step-size', '延拓步长', 'float', '0.001', help='延拓步长，单位 DU。', unit_group='distance', default_unit='DU'),
