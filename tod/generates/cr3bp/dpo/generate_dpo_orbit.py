...
输入为命令行给出的初始状态和周期猜测；输出为 output/dpo/ 下的单条 DPO JSON 文件。
...
DEFAULT_DPO_X0 = 1.03774
DEFAULT_DPO_VY0 = 0.503284
DEFAULT_DPO_PERIOD = 1.2011
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser = argparse.ArgumentParser(
        description="生成 DPO 轨道（手动初值 + 固定周期微分校正）",
    ...
    catalog_group.add_argument("--jacobi", type=float, default=None, help="按目标 Jacobi 值从 normalized DPO catalog 选择 seed")
    catalog_group.add_argument("--seed-id", type=str, default=None, help="按 seed/orbit id 从 normalized DPO catalog 选择 seed")
...
def _ensure_catalog_available(args: argparse.Namespace) -> None:
    ...
    if index_file.exists() and dpo_file.exists():
        return
...
def _select_catalog_seed(args: argparse.Namespace) -> CatalogSeed:
    try:
        ...
        if args.seed_id is not None:
            for record in catalog.records(orbit_type="dpo"):
                if record.orbit_id == args.seed_id:
                    return CatalogSeed(source="catalog_seed_id", record=record)
            raise SystemExit(f"未找到 DPO catalog seed_id={args.seed_id!r}；catalog_dir={args.catalog_dir}")

        record = catalog.nearest_jacobi("dpo", args.jacobi, tolerance=None)
        ...
        raise SystemExit(
            f"DPO catalog 查找失败：catalog_dir={args.catalog_dir}, "
            f"seed_id={args.seed_id!r}, jacobi={args.jacobi!r}, tolerance={args.jacobi_tolerance!r}: {exc}"
        ) from exc
    except Cr3bpImportSchemaError as exc:
        raise SystemExit(f"DPO catalog CSV 无效：catalog_dir={args.catalog_dir}: {exc}") from exc
...
def _save_orbit(orbit: Orbit, metadata: dict[str, object], *, seed_id: str | None = None) -> Path:
    ...
    if seed_id is None:
        ...
        filename = f"dpo_catalog_{_safe_filename_part(seed_id)}_{ts}.json"
...
def main(argv: Sequence[str] | None = None) -> None:
    ...
    target_period = args.period if args.period is not None else DEFAULT_DPO_PERIOD
...
SCRIPT_ENTRY = ScriptEntry(
    ...
    cli_params=[
        CliParam('--x0', '初始 x 坐标', 'float', '1.03774', help='manual 路径初始 x 坐标（无量纲）。', unit_group='distance', default_unit='DU'),
        CliParam('--vy0', '初始 vy 速度', 'float', '0.503284', help='manual 路径初始 y 方向速度（无量纲）。', unit_group='velocity'),
        CliParam('--period', '目标周期', 'float', '1.2011', help='manual 路径目标周期（无量纲）。', unit_group='time', default_unit='days'),
