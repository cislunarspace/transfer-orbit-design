...
可视化 optimize_leo_to_dro.py 的输出结果。
"""
...
def _latest_optimization_json() -> Path | None:
    ...
    candidates = sorted(transfer_dir.glob("optimization_geo_dro_*.json"))
...
def _successful_records(results: list[dict], max_pos_err_km: float) -> list[dict]:
    ...
    for row in results:
        ...
        if pos_violation is not None and np.sqrt(max(0, float(pos_violation))) * DU > max_pos_err_km:
            continue
        success.append(row)
    return success
...
def main() -> None:
    parser = argparse.ArgumentParser(description="可视化 LEO→DRO 优化结果")
