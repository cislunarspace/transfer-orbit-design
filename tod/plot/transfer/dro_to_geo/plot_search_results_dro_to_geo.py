...
def _integrate_single_orbit(args: tuple) -> tuple:
    ...
    DT = 1.0 / (24.0 * tu)
    try:
        system = e2m2e.core.CR3BP_System(
...
SCRIPT_ENTRY = ScriptEntry(
    ...
    cli_params=[
        ...
        CliParam('--scatter-size', '散点大小', 'float', '10', help='散点大小。', advanced=True),
        CliParam('--scatter-alpha', '散点透明度', 'float', '0.7', help='散点透明度。', advanced=True),
        CliParam('--no-title', '隐藏标题', 'bool', '', help='不显示图标题，适合论文配图（图注在论文正文中撰写）。', advanced=True),
